...
func UploadToTempDir(localDir string, remoteOptions *Options) (remoteDir string, err error) {
	...
	if err := client.SFTP().MkdirAll(remoteDir); err != nil {
		return "", fmt.Errorf("Failed to create remote temp directory %s: %s", remoteDir, err)
	}
...
func Upload(localDir string, remoteOptions *Options, remoteDir string) error {
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("Failed to upload %s to %s: %s", localDir, getRemoteSpec(remoteOptions, remoteDir), output)
	}
...
func rsyncCmd(localDir string, remoteOptions *Options, remoteDir string, remoteRsyncPath string) *exec.Cmd {
	...
	// See also putPathAlwaysIgnore in storage/storage.go
...
func ensureRemoteRsync(remoteOptions *Options) (remoteRsyncPath string, err error) {
	...
	if _, err := client.SFTP().Stat(remoteRsyncDir); err != nil {
		if err := client.SFTP().MkdirAll(remoteRsyncDir); err != nil {
			return "", fmt.Errorf("Failed to create remote directory %s: %s", remoteRsyncDir, err)
		}
	}

	remoteFile, err := client.SFTP().Create(remoteRsyncTempPath)
	if err != nil {
		return "", fmt.Errorf("Failed to create remote rsync file at %s: %s", remoteRsyncTempPath, err)
	}

	if _, err := io.Copy(remoteFile, resp.Body); err != nil {
		return "", fmt.Errorf("Failed to write remote rsync file at %s: %s", remoteRsyncTempPath, err)
	}
	if err := remoteFile.Close(); err != nil {
		return "", fmt.Errorf("Failed to close remote rsync file at %s: %s", remoteRsyncTempPath, err)
	}

	if err := client.SFTP().Chmod(remoteRsyncTempPath, 0755); err != nil {
		return "", fmt.Errorf("Failed to make remote rsync file at %s executable: %s", remoteRsyncTempPath, err)
	}
