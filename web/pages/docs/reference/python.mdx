...
These functions implement the core functionality of Replicate, i.e. saving parameters, metrics, and code to storage. You use these functions during model training.
...
- `path`: A path to a file or directory that will be uploaded to storage, relative to the project directory. This can be used to save your training code, or anything you want. If `path` is not set, no data will be saved.
...
- `path`: A path to a file or directory that will be uploaded to storage, relative to the project directory. This can be used to save weights, Tensorboard logs, and other artifacts produced during the training process. If `path` is not set, no data will be saved.
...
- `path`: The path which was saved to storage by `replicate.init()`, relative to `replicate.yaml`
...
- `path`: The path which was saved to storage by `experiment.checkpoint()`, relative to `replicate.yaml`
