...
First, create a new private key pair using the [AWS EC2 console](https://console.aws.amazon.com/ec2/v2/home?region=us-east-1#KeyPairs:). Assuming your key was created with the name "my-aws-key-pair", run:

```shell-session
REGION=us-east-1
IMAGE_ID=$(aws ssm get-parameters --region=$REGION --names "/aws/service/ecs/optimized-ami/amazon-linux-2/recommended/image_id" --query 'Parameters[0].Value' --output text)
aws ec2 run-instances \
        --image-id=$IMAGE_ID \
        --count 1 \
        --instance-type t3.large \
        --key-name=my-aws-key-pair \
        --region=$REGION
```
...
<TabPanel>
First, find the AWS instance ID by running
```shell-session
