...
So far, everything we've been doing has been local on a single machine. But in practice, you probably want to train on a GPU machine, or multiple machines at the same time. Or, perhaps you want to work on the model with other people.

Replicate can store experiment data on cloud storage so you can track all this in one place.

Take a look at [the guide to storing data in the cloud](/docs/tutorial/cloud-storage) to learn more about this.
